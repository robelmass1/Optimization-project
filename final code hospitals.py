...
class Space():
    ...
    def hill_climb(self, maximum=None, image_prefix=None, log=False):
        ...
        optimization_path = [self.hospitals.copy()]
        ...
        while maximum is None or count < maximum:
            ...
            # Append to the optimization path
            optimization_path.append(self.hospitals.copy())

            # Generate image
            if image_prefix:
                self.output_image(f"{image_prefix}{str(count).zfill(3)}.png")

        # Visualize the optimization process on the heatmap
        self.visualize_heatmap(optimization_path)
    ...
    def get_neighbors(self, row, col):
        ...
        return neighbors
    def visualize_heatmap(self, optimization_path):
        """Generates a heatmap without a 3D plot."""
        grid = np.zeros((self.height, self.width))

        # Calculate heat values (sum of Manhattan distances to all houses)
        for i in range(self.height):
            for j in range(self.width):
                grid[i, j] = sum(
                    abs(house[0] - i) + abs(house[1] - j)
                    for house in self.houses
                )
        ...
        for i in range(len(optimization_path) - 1):
            for hospital_start, hospital_end in zip(
                optimization_path[i], optimization_path[i + 1]
            ):
                plt.plot(
                    [hospital_start[1], hospital_end[1]],
                    [hospital_start[0], hospital_end[0]],
                    color='blue', marker='o'
                )
    ...
    def output_image(self, filename):
        ...
        house = Image.open("C:/Users/rkass/Documents/sem 7/foAI/Ad/src3/hospitals/assets/images/House.png").resize(
            (cell_size, cell_size)
        )
        hospital = Image.open("C:/Users/rkass/Documents/sem 7/foAI/Ad/src3/hospitals/assets/images/Hospital.png").resize(
            (cell_size, cell_size)
        )
        font = ImageFont.truetype("C:/Users/rkass/Documents/sem 7/foAI/Ad/src3/hospitals/assets/fonts/OpenSans-Regular.ttf", 30)
